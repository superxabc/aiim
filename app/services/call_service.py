...
class CallManagementService:
    ...
    def join_call(db: Session, call_id: str, user_id: str) -> bool:
        ...
        # 广播加入事件
        try:
            CallManagementService._broadcast_call_event(call, "call.participant_joined", user_id)
        except Exception:
            pass
    ...
    def leave_call(db: Session, call_id: str, user_id: str) -> bool:
        ...
        if remaining_participants == 0:
            ...
            if call and call.status not in ["completed", "missed", "rejected"]:
                ...
                if call.answer_time:
                    call.duration_sec = int((call.end_time - call.answer_time).total_seconds())
        
        db.commit()
        
        # 广播离开事件
        try:
            call = db.query(im_model.CallLog).filter(im_model.CallLog.call_id == call_id).first()
            if call:
                CallManagementService._broadcast_call_event(call, "call.participant_left", user_id)
        except Exception:
            pass
    ...
    @staticmethod
    def _broadcast_call_event(call: im_model.CallLog, event_type: str, user_id: Optional[str] = None):
        """广播通话事件"""
        payload = {
            "event": event_type,
            "call_id": call.call_id,
    ...
    @staticmethod
    def get_active_call(db: Session, conversation_id: str) -> Optional[im_model.CallLog]:
        """获取会话中的活跃通话"""
        return (
            db.query(im_model.CallLog)
            .filter(
