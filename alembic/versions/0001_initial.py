...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('conversation_id', sa.String(), sa.ForeignKey('conversations.conversation_id'), index=True),
        sa.Column('user_id', sa.String(), index=True),
        sa.Column('role', sa.Enum('owner','member','assistant', name='conversation_member_role'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('type', sa.Enum('text','image','file','audio','video','system','ai','stream_chunk', name='message_type'), nullable=False),
    ...
    op.create_index('idx_messages_sender_created', 'im_messages', ['sender_id','created_at'])
